...
OUTPUTS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'outputs')
...
class OutputPlugin:
    ...
    def save_partial_progress(self, adr_id: str, section:str, text: str) -> str:
        ...
        """
        os.makedirs(os.path.join(OUTPUTS_DIR, f"{adr_id}"), exist_ok=True)
        short_uuid = str(uuid.uuid4())[:5]
        kebab_section = to_kebab_case(section)
        file_path = os.path.join(OUTPUTS_DIR, f"{adr_id}", f"{kebab_section}-{short_uuid}.md")
